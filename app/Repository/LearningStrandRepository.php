...
/**
 * Data access for `learning_strands` (สาระการเรียนรู้ —
 ...
 * this module.
 */
final class LearningStrandRepository extends AbstractRepository
{
    protected function table(): string
    {
        return 'learning_strands';
    }
    ...
    public function create(array $data): string|int
    {
        $this->connection->execute(
            'INSERT INTO learning_strands (subject_code, strand_code, strand_name_th) VALUES (?, ?, ?)',
            [$data['subject_code'], $data['strand_code'], $data['strand_name_th']],
        );
    ...
    public function update(string|int $id, array $data): bool
    {
        ...
        $statement = $this->connection->execute(
            "UPDATE learning_strands SET {$assignments} WHERE id = ?",
    ...
    {
        $statement = $this->connection->execute('DELETE FROM learning_strands WHERE id = ?', [$id]);
