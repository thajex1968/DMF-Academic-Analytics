...
| 2.0.9 | 2026-07-04 | Added an eighth Post-Freeze Amendments entry: documentation alignment with approved [docs/rfcs/RFC-004](rfcs/RFC-004-multi-source-analytics-architecture.md) (Multi-Source Analytics Architecture) across `01-PRD.md`, `02-System-Architecture.md`, `03-Database-Design.md`, `Data-Dictionary.md`, `Business-Flow.md`, `IMPLEMENTATION_GUIDE.md`, and `PROJECT_BOARD.md` — no schema, functional requirement, Sprint task, or v1.0 scope change; every edit is a wording/labeling clarification. Also retroactively added the previously-missing table row for the seventh amendment above (its version bumps had already shipped; the row itself was never written). | DMF Platform Team |
| 2.0.10 | 2026-07-04 | Added a ninth Post-Freeze Amendments entry, made during the `v0.5.0` "Analytics Engine Complete" release-hardening pass (Sprint 4 complete: Analytics Domain Foundation, Analytics Calculators, Analytics Aggregation & Dashboard Data API): `Release-Notes.md` (→1.1.0) rewritten to match real shipped state — it had drifted to say only T1.1/T1.2 were done and carried no entry at all for the three real, tagged releases `v0.2.0`–`v0.4.0`; now every entry through `v0.5.0` accurately says Released vs. Planned. No scope, requirement, or architecture change — a documentation-accuracy correction found during this pass's Documentation Audit (Task 7). | DMF Platform Team |
...
| 7 | 2026-07-03 | Made during T2.6 (Duplicate Detection + Audit Trail, FR-007/FR-008) implementation: `import_logs.event`'s vocabulary extended from 6 to 10 values, adding `duplicate_found`, `import_started`, `retry`, `rollback`. See [decisions/IDR-008](../decisions/IDR-008-import-audit-event-vocabulary-extension.md). *(Row added retroactively during the RFC-004 alignment pass below — this entry's version bump had already shipped via §Revision History v2.0.8, but the table row itself was missed at the time.)* | `03-Database-Design.md` (→2.0.2); `Data-Dictionary.md` (→1.0.2); `00-Project-Overview.md` (→2.0.8) |
| 8 | 2026-07-04 | Documentation alignment with approved [docs/rfcs/RFC-004](rfcs/RFC-004-multi-source-analytics-architecture.md) (Multi-Source Analytics Architecture), done ahead of any Sprint 4/Phase 3 implementation, per that RFC's own recommendation to adopt its Level 1/2/3 Assessment Data Classification, Source Independence, Assessment Adapter Layer, and Canonical Analytics Model vocabulary consistently across the document set — **no schema, migration, functional requirement, Sprint task, or v1.0 scope change**; every edit is a wording/labeling clarification, several made at RFC-004's own explicit direction (its Analysis §6 names the exact PRD/architecture/database sections needing this pass). `student_question_responses`'s documented producer corrected from an implied O-NET source to "any Level 2 source — none built yet." `question_analysis.discrimination_index`/`.distractor_frequency_json` and `.difficulty_index` (classroom scope) labeled **Reserved for Future Assessment Sources**; `.difficulty_index` (school scope) confirmed Level 1-sufficient/in-scope-now. `standard_performance_summary`'s indicator-vs-standard grain mismatch given an explicit open-question note deferring to a future ADR, without presuming that ADR's answer. Named the **Assessment Adapter Layer** and **Canonical Analytics Model** as the architecture's source-independence extension point, grounded in the existing Normalization module (T2.5), with RFC-004's diagram. | `03-Database-Design.md` (→2.0.3); `Data-Dictionary.md` (→1.0.3); `01-PRD.md` (→2.0.3); `02-System-Architecture.md` (→2.0.4); `Business-Flow.md` (→2.0.1); `IMPLEMENTATION_GUIDE.md` (→1.2.0); `PROJECT_BOARD.md` (Backlog pointer added, no version field to bump — living tracker); `Release-Notes.md` (reviewed, no change required — does not yet cover Phase 3/Sprint 4 content); `00-Project-Overview.md` (→2.0.9, this entry) |
| 9 | 2026-07-04 | Rewrote [Release-Notes.md](Release-Notes.md) during the `v0.5.0` "Analytics Engine Complete" release-hardening pass — found, via that pass's Documentation Audit, that it had drifted to say only T1.1/T1.2 (of Sprint 1) were complete and had no entry at all for `v0.2.0`-`v0.4.0`, despite all three being real, tagged git releases. Every entry through the new `v0.5.0` now accurately reflects Released vs. Planned status, verified against git tags and [PROJECT_BOARD.md](../PROJECT_BOARD.md). No scope, requirement, or architecture change. | `Release-Notes.md` (→1.1.0); `00-Project-Overview.md` (→2.0.10, this entry); `VERSION` (root, →0.5.0) |
